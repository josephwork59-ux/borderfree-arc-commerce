...
const Home = () => {
  ...
  useEffect(() => {
    isScollable();
    window.addEventListener('resize', isScollable);

    return () => {
      window.removeEventListener('resize', isScollable);
    };
  });
  ...
  return (
    <div className="flex justify-center sm:px-4 p-12">
      <div className="w-full minmd:w-4/5">
        <Banner
          name={(<>Explore, buy, and sell<br />extraordinary NFTs</>)}
          childStyles="md:text-4xl sm:text-2xl xs:text-xl text-left"
          parentStyles="justify-start mb-6 h-72 sm:h-60 p-12 xs:p-4 xs:h-44 rounded-3xl"
        />

        {!isLoading && !nfts.length ? (
          <h1 className="font-poppins dark:text-white text-nft-black-1 text-2xl minlg:text-4xl font-semibold ml-4 xs:ml-0">That&apos;s interesting... No NFTs for sale!</h1>
        ) : isLoading ? <Loader /> : (
          <>
            <div>
            ...
            <div className="relative flex-1 max-w-full flex mt-3" ref={parentRef}>
              <div className="flex flex-row w-max overflow-x-scroll no-scrollbar select-none" ref={scrollRef}>
                {topCreators.map((creator, i) => (
                  // import a customer component from utils folder 
                  <CreatorCard
                    key={creator.seller}
                    rank={i + 1}
